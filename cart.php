...
//     unset($_SESSION['cart']);
//     header('Location: process_order.php'); // Change this line to redirect to order.php
//     exit;
// }

if (isset($_POST['placeorder']) && isset($_SESSION['cart']) && !empty($_SESSION['cart'])) {
    // Store the cart data in a separate session variable before clearing it
    $_SESSION['order_details'] = $_SESSION['cart'];

    // Clear the cart after storing the order details
    unset($_SESSION['cart']);

    header('Location: process_order.php'); // Redirect to process_order.php
    exit;
}
